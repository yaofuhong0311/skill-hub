...
**内置配置不含任何具体 skill 信息**，只保留分类框架（categories 数组）+ 兜底规则。理由（用户提出）：v1~v4 把作者本机 110 个 skill 的分类/简介/模板当"内置默认"打包，等于把作者装了哪些 skill（含公司内部的）公开，且对别人机器上的 skill 也对不上号。

正解：
- `config/categories.json` = `{categories:[8个分类定义], mapping:{}}`；`summaries.json`/`templates.json` = `{}`
- 每个用户针对自己扫到的 skill，用「补全配置 Prompt」生成自己的三份 JSON，落在 `~/Library/Application Support/skill-hub/`
- 作者本人的 110 条完整配置已迁到自己的用户目录，本机使用无损
- 一个 dmg 通吃：作者装＝读到自己用户目录（完整）；他人装＝用户目录空→引导补全
